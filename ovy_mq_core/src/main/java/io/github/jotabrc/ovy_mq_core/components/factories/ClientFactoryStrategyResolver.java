...
public class ClientFactoryStrategyResolver {
    ...
    public Client create(DefinitionMap definition) {
        Optional<ClientFactoryStrategy> strategy = get(definition.extract(Key.HEADER_CLIENT_TYPE, ClientType.class));
        return strategy
                .map(clientFactoryStrategy -> clientFactoryStrategy.create(definition))
                .orElseThrow(() -> new OvyException.ConfigurationError("Unable to create factory for=%s".formatted(definition.extract(Key.HEADER_CLIENT_TYPE, ClientType.class))));
    }
